...
import { CheckSelectedIcon } from "../Icons";
...
export function ProfileMenu({ token, profileName }) {
  ...
  const logoutUser = async () => {
    // const low = await ProxyLogout(token);
    signOut();
  };
      ...
      <Popover>
        {({ open }) => (
          <>
            ...
            >
              <Popover.Panel className="absolute -translate-x-1/2 z-100 mt-2 px-4">
                <div className="overflow-hidden rounded-sp-14 shadow-lg bg-sp-day-300 border-sp-fawn dark:bg-sp-black border-2 dark:border-sp-medium text-sp-black dark:text-sp-white">
                  <div className="flex flex-col justify-evenly gap-y-2 p-3">
                    <a
                      key={"dark"}
                      className="flex w-52 justify-start items-center rounded-sp-14 p-4 hover:bg-sp-day-50 dark:hover:bg-gradient-to-r dark:hover:from-sp-dark-brown dark:hover:to-sp-brown focus:outline-none"
                    >
