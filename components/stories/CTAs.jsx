...
export function SearchPlacesCTA () {
  ...
  return (
    <div className='container w-full xl:w-4/5 mx-auto border-3 dark:border-3 border-sp-day-200 dark:border-sp-fawn dark:border-opacity-10 rounded-xl justify-center items-center text-sp-black dark:text-sp-white my-10'>
      <div className='flex flex-col items-center mx-auto py-16 px-4 sm:px lg:py-20 lg:px-8'>
        <div className='bg-sp-day-900 bg-opacity-10 dark:bg-sp-dark-brown rounded-xl p-1.5 mx-2 mb-2'>
          <LocationIcon width={8} height={8} />
        </div>
        <h2 className='text-xl sm:text-xl md:text-2xl lg:text-3xl font-extrabold mb-4'>
          {t('search_places_title')}
        </h2>
        <div className='flex flex-wrap w-3/5 justify-center gap-2 items-center mx-auto pt-3 pb-6'>
          {places.map(p => {
            return <PlacePill name={p.name} count={p.count} key={p.name} />
          })}
        </div>
...
export function PlacePill ({ name, count }) {
  return (
    <a
      href='/'
      className='bg-gradient-to-r from-sp-day-300 to-sp-day-100 dark:from-sp-dark-brown dark:to-sp-brown focus:outline-none inline-flex justify-between items-center font-semibold rounded-full py-2 px-3 gap-1'
    >
      <p>{name}</p>
