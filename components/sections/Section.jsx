...
export function SectionGrid ({ totalPages, title, isLastPage, initialItems }) {
  ...
  return (
    <div className='container w-full xl:w-3/5 mx-auto flex flex-col items-center my-16'>
      <div className='flex flex-col items-center mb:14 lg:mb-20'>
        ...
        <div className='inline-flex mt-6 items-center gap-3'>
          ...
          <div className='border-r-3 h-4 w-1 border-sp-brown rounded-sm'></div>
      ...
      {!isLast && (
        <button className='dark:bg-sp-medlight border border-sp-lighter dark:border-sp-medium hover:bg-gradient-to-r from-sp-day-300 to-sp-day-100 dark:hover:from-sp-dark-brown dark:hover:to-sp-brown focus:outline-none inline-flex items-center gap-1 rounded-full py-4 px-8 font-semibold'>
          {t('action_load_more')}
        </button>
      )}
    </div>
...
function PlaceHolderTile ({ story_id, text }) {
  return (
    <Link href={`/story/id/${story_id}`} key={story_id}>
...
function SectionStoryTile ({
  ...
  return (
    <Link href={`/story/id/${story_id}`} key={title}>
      <a className='group'>
