...
export async function GetMovieById(id, accessToken, lang) {
  let res;
  if (accessToken) {
    res = await axios.get(`${MOVIE_API}/movies/${id}`, {
      headers: {
...
export async function GetMovies(accessToken) {
    let res;
    if (accessToken) {
      res = await axios.get(`${MOVIE_API}/movies/${id}`, {
        headers: {
      ...
      res = await axios.get(`${MOVIE_API}/movies`);
    }
